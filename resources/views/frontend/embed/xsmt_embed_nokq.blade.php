...
<head>
    ...
    <link rel="canonical" href="{{route('xsmt.date',getNgayLink($date))}}"/>
    <link rel="alternate" href="{{route('xsmt.date',getNgayLink($date))}}" hreflang="vi-vn">
    <link rel="alternate" href="{{route('xsmt.date',getNgayLink($date))}}" hreflang="x-default">
...
<div class="rd bg-white" id='mt_kqngay_{{getNgayID($date)}}' onclick="window.open('{{route('xsmt')}}','_blank');return;">
    <div class="kqsx-old-mn">
        <div class="kqsx-tinh kqsx mb">
            <table class="tbl-kqsx text-center">
                <tbody>
                <tr>
                    <th @if($count==2) colspan="19" @else colspan="27" @endif>
                        <h2 class="h-kqsx">Xổ Số miền Trung (XSMT) ngày {{getNgay($date)}}</h2>
                        <h3 class="breadcrumb-link">
                            <a href="{{route('xsmt')}}">XSMT</a> / <a href="{{route(getRouteDay(getThuNumber($date),'xsmt'))}}">XSMT {{getThu(getThuNumber($date))}}</a> / <a
                                    href="{{route('xsmt.date',getNgayLink($date))}}">XSMT {{getNgay($date)}}</a>
                        </h3>
                    </th>
                </tr>
                <tr class="odd-bg">
