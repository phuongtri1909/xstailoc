...
<head>
    ...
    <link rel="canonical" href="{{route('xsmb.date',getNgayLink($date))}}"/>
    <link rel="alternate" href="{{route('xsmb.date',getNgayLink($date))}}" hreflang="vi-vn">
    <link rel="alternate" href="{{route('xsmb.date',getNgayLink($date))}}" hreflang="x-default">    
...
<body>
<div class="box bg-white rd shadow" id='kqngay_{{getNgayID($date)}}' onclick="window.open('{{route('xsmb')}}','_blank');return;">
    <div class="kqmb shadow kqsx mb">
        <table class="table-kqsx">
            <thead>
            <tr>
                <th colspan="27" class="header-kqsx">
                    <h2 class="h-kqsx">XSMB {{getNgayThang($date)}}, Xổ Số Miền Bắc Ngày {{getNgay($date)}}</h2>
