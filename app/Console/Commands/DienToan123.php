...
class DienToan123 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'DienToan123:everyMinute';
    ...
    {
        set_time_limit(0);
        Log::info('Get Dien toan 123 Start:' . date('Ymd H:i:s'));
        ...
        for ($k = 0; $k <= 2; $k++) {
            ...
            $count = DienToan::where('date', $date)->where('type', 2)->where('status', 1)->count();
            if ($count > 0) continue;
            ...
            $check = DienToan::where('date', $date)->where('type', 2)->count();
            if ($check == 0) {
        ...
        Log::info('Get Dien toan 123 End:' . date('Ymd H:i:s'));
    }
    ...
    public function fire()
    {
        ...
        Bus::dispatchFromArray('App\Console\Commands\DienToan123', []);
    }
}
