...
class GetXSMT extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getXsmt:everyMinute';
    ...
    {
        Log::info('KQMT KQVS Start:' . date('Ymd H:i:s'));
        $url = 'https://s1.ketquaveso.mobi/ttkq/json_kqmt/0fb41e71bc903b627e9842ff5e6f2901';
        $kqs = requestvl($url);

        if (empty($kqs)) {
            return $this->getKqXsmt();
        }
        ...
        foreach ($kqs as $kq) {
            ...
            $check = Lottery::where('province_id', $province->id)->where('date', $date)->where('mien', 2)->count();
            if ($check == 0) {
                Lottery::create([
                ...
                Lottery::where('province_id', $province->id)->where('date', $date)->where('mien', 2)
                    ->update([
                        'gdb' => preg_replace('/\s+/', '', $giai['DB']),
                        'g1' => preg_replace('/\s+/', '', $giai[1]),
                        'g2' => preg_replace('/\s+/', '', $giai[2]),
        ...
        Log::info('KQMT KQVS End:' . date('Ymd H:i:s'));
    }


    public function getKqXsmt()
    {
        Log::info('KQMT Start:' . date('Ymd H:i:s'));
...
//        $rootPath = substr($rootPath, 16, strpos($rootPath, "';") - 16);
        ...
        Log::info('KQMT End:' . date('Ymd H:i:s'));
    }


    public function getXsmtHTML()
    {
        Log::info('KQMT HTML Start:' . date('Ymd H:i:s'));
        set_time_limit(0);
...
//            $province = Province::where('slug', $slug)->first();
            ...
            $check = Lottery::where('province_id', $province->id)->where('date', $ngay_quay)->where('mien', 2)->count();
            if ($check == 0) {
                ...
                Lottery::where('province_id', $province->id)->where('date', $ngay_quay)->where('mien', 2)
                    ->update([
                        'gdb' => preg_replace('/\s+/', '', $sxmn[8]),
                        'g1' => preg_replace('/\s+/', '', $sxmn[7]),
                        'g2' => preg_replace('/\s+/', '', $sxmn[6]),
        ...
        Log::info('KQMT HTML End:' . date('Ymd H:i:s'));
    }
    ...
    public function fire()
    {
        ...
        Bus::dispatchFromArray('App\Console\Commands\GetXSMT', []);
    }
}
